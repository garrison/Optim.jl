...
function x_trace(r::UnivariateOptimizationResults)
	tr = trace(r)
	!haskey(tr[1].metadata, "current_minimizer") && error("Trace does not contain x. To get a trace of x, run optimize() with extended_trace = true")
    [ state.metadata["current_minimizer"] for state in tr ]
end
